  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: colors.background, color: colors.text, fontFamily: typography.bodyFont }}>
      <section className="min-h-screen flex flex-col items-center justify-center text-center px-6">
        ...
        <p className="text-xs uppercase tracking-[0.35em] mb-4" style={{ color: colors.textMuted }}>{t([1047,1072,1087,1088,1086,1096,1091,1108,1084,1086,32,1074,1072,1089,32,1085,1072,32,1085,1072,1096,1077,32,1074,1077,1089,1110,1083,1083,1103])}</p>
        <h1 className="text-6xl md:text-8xl leading-tight" style={{ fontFamily: typography.headingFont }}>{wedding.partner_name_1}<br /><span style={{ color: colors.primary, fontFamily: typography.accentFont, fontStyle: 'italic' }}>&amp;</span><br />{wedding.partner_name_2}</h1>
      ...
      {wedding.wedding_date && <Countdown date={wedding.wedding_date} colors={colors} typography={typography} />}
      {venue.name && (
        <section className="py-20 px-6 text-center" style={{ backgroundColor: colors.surface }}>
          <p className="text-xs uppercase tracking-[0.25em] mb-3" style={{ color: colors.primary }}>{t([1052,1110,1089,1094,1077,32,1087,1088,1086,1074,1077,1076,1077,1085,1085,1103])}</p>
      ...
      <section className="py-20 px-6">
        <div className="max-w-md mx-auto text-center">
          <p className="text-xs uppercase tracking-[0.25em] mb-3" style={{ color: colors.primary }}>{t([1055,1088,1086,1075,1088,1072,1084,1072,32,1076,1085,1103])}</p>
          <h2 className="text-3xl md:text-4xl mb-10" style={{ fontFamily: typography.headingFont }}>{t([1056,1086,1079,1082,1083,1072,1076])}</h2>
          <div className="space-y-8">
            {wedding.ceremony_time && <TL time={formatTime(wedding.ceremony_time, wedding.locale)} label={t([1062,1077,1088,1077,1084,1086,1085,1110,1103])} c={colors} ty={typography} />}
            {schedule.receptionTime && <TL time={schedule.receptionTime} label={t([1060,1091,1088,1096,1077,1090])} c={colors} ty={typography} />}
            {schedule.dinnerTime && <TL time={schedule.dinnerTime} label={t([1042,1077,1095,1077,1088,1103])} c={colors} ty={typography} />}
            {schedule.partyTime && <TL time={schedule.partyTime} label={t([1042,1077,1095,1110,1088,1082,1072])} c={colors} ty={typography} />}
          </div>
        </div>
      </section>
      ...
      <section className="py-16 px-6" style={{ backgroundColor: colors.surface }}>
        <div className="max-w-md mx-auto grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Link href={'/w/' + wedding.slug + '/camera'} className="flex flex-col items-center gap-2 py-8 rounded-xl border hover:shadow-md" style={{ borderColor: colors.border, backgroundColor: colors.background }}>
            <span className="text-3xl">{"\ud83d\udcf8"}</span>
            <span className="text-sm font-medium" style={{ color: colors.text }}>{t([1043,1086,1089,1090,1100,1086,1074,1072,32,1082,1072,1084,1077,1088,1072])}</span>
          </Link>
          <Link href={'/w/' + wedding.slug + '/guestbook'} className="flex flex-col items-center gap-2 py-8 rounded-xl border hover:shadow-md" style={{ borderColor: colors.border, backgroundColor: colors.background }}>
            <span className="text-3xl">{"\ud83d\udcd6"}</span>
            <span className="text-sm font-medium" style={{ color: colors.text }}>{t([1050,1085,1080,1075,1072,32,1075,1086,1089,1090,1077,1081])}</span>
          </Link>
          <Link href={'/w/' + wedding.slug + '/booth'} className="flex flex-col items-center gap-2 py-8 rounded-xl border hover:shadow-md" style={{ borderColor: colors.border, backgroundColor: colors.background }}>
            <span className="text-3xl">{"\ud83c\udfad"}</span>
            <span className="text-sm font-medium" style={{ color: colors.text }}>{t([1060,1086,1090,1086,1073,1091,1076,1082,1072])}</span>
          </Link>
        </div>
      </section>
      <section className="py-16 px-6 text-center" style={{ backgroundColor: colors.background }}>
        <p className="text-2xl" style={{ fontFamily: typography.accentFont, color: colors.primary }}>{t([1063,1077,1082,1072,1108,1084,1086,32,1085,1072,32,1074,1072,1089,33])}</p>
...
function Countdown({ date, colors, typography }: any) {
  ...
  useEffect(() => { const i = setInterval(() => setCd(getCountdown(date)), 1000); return () => clearInterval(i); }, [date]);
  if (cd.isPast) return null;
  return (
    <section className="py-20 px-6 text-center">
      <p className="text-xs uppercase tracking-[0.25em] mb-8" style={{ color: colors.primary }}>{t([1044,1086,32,1074,1077,1089,1110,1083,1083,1103,32,1079,1072,1083,1080,1096,1080,1083,1086,1089,1100])}</p>
      <div className="flex items-center justify-center gap-6 md:gap-10">
        {[{v:cd.days,l:t([1076,1085,1110,1074])},{v:cd.hours,l:t([1075,1086,1076,1080,1085])},{v:cd.minutes,l:t([1093,1074,1080,1083,1080,1085])},{v:cd.seconds,l:t([1089,1077,1082,1091,1085,1076])}].map(i=>(
          <div key={i.l}>
            <div className="text-4xl md:text-5xl font-light tabular-nums" style={{fontFamily:typography.headingFont,color:colors.text}}>{String(i.v).padStart(2,'0')}</div>
            <div className="text-[10px] uppercase tracking-widest mt-1" style={{color:colors.textMuted}}>{i.l}</div>
          </div>
        ))}
      </div>
    </section>
...
function TL({time,label,c,ty}:any){
  return(
    <div className="flex items-center gap-6">
      <div className="text-lg w-16 text-right tabular-nums" style={{fontFamily:ty.accentFont,color:c.primary}}>{time}</div>
      <div className="w-2 h-2 rounded-full flex-shrink-0" style={{backgroundColor:c.primary}}/>
      <div className="text-sm text-left" style={{color:c.text}}>{label}</div>
    </div>
  );
      }'use client';
import { useState, useEffect } from 'react';
    ...
    <div className="min-h-screen" style={{ backgroundColor: colors.background, color: colors.text, fontFamily: typography.bodyFont }}>
      ...
      {venue.name && (
        <section className="py-20 px-6 text-center" style={{ backgroundColor: colors.surface }}>
          ...
          <h2 className="text-3xl md:text-4xl mb-4" style={{ fontFamily: typography.headingFont }}>{venue.name}</h2>
          <p className="text-sm" style={{ color: colors.textMuted }}>{venue.address}</p>
        </section>
      )}
      <section className="py-20 px-6">
        <div className="max-w-md mx-auto text-center">
      ...
      <section className="py-16 px-6" style={{ backgroundColor: colors.surface }}>
        <div className="max-w-md mx-auto grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <Link href={'/w/' + wedding.slug + '/booth'} className="flex flex-col items-center gap-2 py-8 rounded-xl border transition-colors hover:shadow-md" style={{ borderColor: colors.border, backgroundColor: colors.background }}>
            <span className="text-3xl">{"🎭"}</span>
            <span className="text-sm font-medium" style={{ color: colors.text }}>{"Фотобудка"}</span>
          </Link>
        </div>
      </section>
      <section className="py-16 px-6 text-center" style={{ backgroundColor: colors.background }}>
