...
export default function LoginPage() {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-6">
      <div className="max-w-sm w-full text-center">
        <Link href="/" className="font-serif font-semibold text-2xl text-[#1a1a2e] tracking-wide">Momently<span className="text-[#b8956a]">.</span></Link>
        <h1 className="font-serif text-3xl text-[#1a1a2e] mt-8">Увійти</h1>
        <p className="text-sm text-gray-500 mt-2">Увійдіть, щоб керувати своїм весіллям</p>
        ...
        <div className="mt-8 space-y-3">
          ...
          <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="email@example.com" onKeyDown={e => e.key === 'Enter' && handleMagicLink()} className="w-full px-4 py-3 border border-[#e8e0d4] rounded-lg text-sm focus:outline-none focus:border-[#b8956a]" />
          <button onClick={handleMagicLink} disabled={loading || !email} className="w-full bg-[#1a1a2e] text-[#faf8f4] rounded-lg px-4 py-3 text-sm font-medium hover:bg-[#2a2a3e] transition-colors disabled:opacity-50">{loading ? 'Надсилаємо...' : 'Отримати magic link'}</button>
        </div>
        <p className="text-xs text-gray-400 mt-6">Ще немає акаунту? <Link href="/auth/signup" className="text-[#b8956a] hover:underline">Зареєструватись</Link></p>
      </div>
    </main>
  );
}
